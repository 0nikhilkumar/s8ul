...
function extractPlayers(raw, category, team) {
    ...
    const match = list.find((teamData) => {
        const catMatch = teamData.categoryName?.toLowerCase() === category?.toLowerCase();
        const teamMatch = !team || teamData.teamName?.toLowerCase() === team?.toLowerCase();
    ...
    return list
        .filter((teamData) => teamData.categoryName?.toLowerCase() === category?.toLowerCase())
...
function Esports() {
    ...
    const handleBGMILineupChange = (lineup) => {
        if (lineup === activeBGMILineup) return;
